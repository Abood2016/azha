            ...
            <x-slot name="header">
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                    <div class="w-100 card-toolbar justify-content-between">
                        ...
                        <!--end::Search Form-->
                        <!--begin::Button-->
                        <x-metronic.button-link href="{{ route('sliders.create') }}">
...
@section('scripts')
<script>
    <x-metronic.datatable.table data="{{ route('sliders.index') }}" search-columns="name">
        <x-slot name="columns">
        ...
        <x-metronic.datatable.column field="is_active" title="{{ __('Status') }}">
            <x-slot name="template">
                <span class="label font-weight-bold label-lg label-light-${row.is_active === 1 ? 'success' : 'danger'} label-inline">
                    ${row.is_active === 1 ? 'مفعل' : 'حظر'}
                </span>
            </x-slot>
        </x-metronic.datatable.column>
        ...
        <x-slot name="actions">
            <x-metronic.datatable.actions-drop-down>
                <x-metronic.datatable.actions-drop-down-item href="sliders/${row.id}/edit">
                    <span class="svg-icon">
                ...
                <x-metronic.datatable.actions-drop-down-item href="javascript:;" data-id="${row.id}" class="w-100 makeActive">
                    <span class="svg-icon">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                            <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                <rect x="0" y="0" width="24" height="24"></rect>
        ...
        <x-slot name="extra">
            $('body').on('click', '.delete', function() {
                ...
                axios.delete("/sliders/"+$(this).data("id"))
            ...
            $('body').on('click', '.makeActive', function() {
                ...
                axios.post("/sliders/"+$(this).data("id")+"/make-active")
                    .then(function (response) {
                    <x-metronic.datatable.reload />
                    })
                    .catch(function (error) {
