            ...
            <x-slot name="header">
                <div x-data="{ open: false }" class="card-header flex-wrap border-0 pt-6 pb-0">
                    <div class="w-100 card-toolbar justify-content-between">
                        ...
                        <!--end::Search Form-->
                        <!--begin::Button-->
                        <x-metronic.button-link href="{{ route('drivers.create') }}">
...
<script>
    ...
    <x-metronic.datatable.table data="{{ route('metronic.drivers') }}" search-columns="user.name, user.phone" relation="user">
        <x-slot name="columns">
            <x-metronic.datatable.column field="user.name" title="{{ __('Name') }}" sort="off">
            ...
            <x-metronic.datatable.column field="phone" title="{{ __('Phone') }}" sort="off">
                <x-slot name="template">
                    ${row.user.phone}
                </x-slot>
            </x-metronic.datatable.column>
            <x-metronic.datatable.column field="orders_count" title="{{ __('Orders') }}" sort="off">
                <x-slot name="template">
                    ${`<x-metronic.button-link href="/driver/${row.id}/orders">
                           Show ${row.orders_count}
                        </x-metronic.button-link>`}
                </x-slot>
            </x-metronic.datatable.column>
            <x-metronic.datatable.column field="revenue" title="{{ __('Revenue') }}" sort="off">
                <x-slot name="template">
                    ${row.revenue}
                </x-slot>
            </x-metronic.datatable.column>
            <x-metronic.datatable.column field="commissions" title="{{ __('Commissions') }}" sort="off">
                <x-slot name="template">
                    ${row.commissions}
                </x-slot>
            </x-metronic.datatable.column>
            <x-metronic.datatable.column field="rate" title="{{ __('Rate') }}" sort="off">
                    <x-slot name="template">
                        ...
                        ${`<span>${row.rate} ${` <span> (${row.times_rated})</span>`}</span>`}
                    </x-slot>
            </x-metronic.datatable.column>
            ...
            <x-metronic.datatable.column field="approve" title="{{ __('Verified') }}">
                <x-slot name="template">
                    <span class="label font-weight-bold label-lg label-light-${row.approve === 0 ? 'dark' : (row.approve === 1 ? 'primary' : 'danger')} label-inline">
                        ${row.approve === 0 ? 'Pending' : (row.approve === 1 ? 'Approved' : 'Declined')}
                    </span>
                </x-slot>
            </x-metronic.datatable.column>


            <x-metronic.datatable.column field="available" title="{{ __('Availability') }}">
                <x-slot name="template">
                    <span class="label font-weight-bold label-lg label-light-${row.available === 1 ? 'success' : 'danger'} label-inline">
                        ${row.available === 1 ? 'Available' : 'Unavailable'}
                    </span>
                </x-slot>
            </x-metronic.datatable.column>

        <x-metronic.datatable.column field="connection" title="{{ __('Connection') }}">
            <x-slot name="template">
            <span class="label font-weight-bold label-lg label-light-${row.connection === 1 ? 'success' : 'danger'} label-inline">
            ${row.connection === 1 ? 'Online' : 'Offline'}
            </span>
            </x-slot>
        </x-metronic.datatable.column>

            <x-metronic.datatable.column field="created_by" title="{{ __('Created By') }}">
                <x-slot name="template">
                    ${row.created_by}
                </x-slot>
            </x-metronic.datatable.column>
        ...
        <x-slot name="actions">
            <x-metronic.datatable.actions-drop-down>
                <x-metronic.datatable.actions-drop-down-item href="drivers/${row.id}">
                    <span class="svg-icon">
                ...
                <x-metronic.datatable.actions-drop-down-item href="drivers/${row.id}/edit">
                    <span class="svg-icon">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                            <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                <rect x="0" y="0" width="24" height="24"/>
                ...
                <x-metronic.datatable.actions-drop-down-item href="javascript:;" data-id="${row.id}" data-update="approve" class="w-100 update">
                    ${row.approve === 1 ?
                        `<span class="svg-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                                <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
        ...
        <x-metronic.datatable.actions-drop-down-item href="javascript:;" data-id="${row.id}" data-update="available" class="w-100 update">
                        ${(row.available === 0 && row.lastTrip == 2) ?
                                 `<span class="svg-icon">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="24px" viewBox="0 0 24 24" version="1.1">
                            <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                <rect x="0" y="0" width="24" height="24"/>
        ...
        <x-slot name="extra">
            ...
            $('body').on('click', '.update', function() {
                axios.post("{{ route('metronic.driver.status') }}", {
                        id: $(this).data("id"),
            ...
            $('body').on('click', '.delete', function() {
                ...
                var users = null;
                axios.post("{{ route('metronic.driver.destroy') }}", {
                        id: $(this).data("id"),
                    })
                    .then(function (response) {
                        <x-metronic.datatable.reload />
