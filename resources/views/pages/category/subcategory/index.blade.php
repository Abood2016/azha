...
{{--                            <!--begin::Button-->--}}
{{--                            <x-metronic.button-link href="{{ route('sub-categories.create') }}">--}}
...
@section('scripts')
    <script>
        <x-metronic.datatable.table data="{{ route('sub-categories.index') }}" search-columns="name,id">
            <x-slot name="columns">
                <x-metronic.datatable.column field="name" title="{{ __('القسم الفرعي') }}">
                    <x-slot name="template">
                        ${row.name}
                    </x-slot>

                </x-metronic.datatable.column>
                <x-metronic.datatable.column field="category" title="{{ __('القسم الرئيسي') }}">
                    <x-slot name="template">
                        ${row.category}
                    </x-slot>
                </x-metronic.datatable.column>

                <x-metronic.datatable.column field="active" title="{{ __('الحالة') }}">
                    <x-slot name="template">
                    <span
                        class="label font-weight-bold label-lg label-light-${row.active === 1 ? 'success' : 'danger'} label-inline">
                        ${row.active === 1 ? 'مفعل' : 'غير مفعل'}
                    </span>
                    </x-slot>
                </x-metronic.datatable.column>
            </x-slot>
            <x-slot name="actions">
                ...
                <x-metronic.datatable.actions-drop-down>
                    <x-metronic.datatable.actions-drop-down-item href="sub-categories/${row.id}/edit">
                    <span class="svg-icon">
            ...
            <x-slot name="extra">
                ...
                axios.delete("/sub-category/"+$(this).data("id"))
                ...
                $.ajax({
                    url: "sub-categories/" + $(this).data("id"),
                    type: 'DELETE',
